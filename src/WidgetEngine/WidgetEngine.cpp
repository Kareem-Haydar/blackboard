...
namespace WidgetEngine {
  ...
  void Engine::AddLabel(const std::string& window, const std::string& name, const std::string& text, const WidgetAlignment& alignment, bool elevated) {
    WidgetEngine::WindowHandle* handle = GetWindow(window);
    if (handle && handle->window && handle->window->isWindow()) {
  ...
  void Engine::AddAnimation(const std::string& name, const std::string& window, const std::string& widget, const std::string& property, unsigned int duration, int curve) {
    WindowHandle* win = GetWindow(window);
    if (win && win->window && win->window->isWindow()) {
